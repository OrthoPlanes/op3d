...
def iterate_random_cond(opts, batch_size):
    dataset = dnnlib.util.construct_class_by_name(**opts.dataset_kwargs)
    while True:
        cond = [dataset.get_conditions(np.random.randint(len(dataset))) for _i in range(batch_size)]
        yield_cond = {}
        for key in cond[0].keys():
            tmp_value = [torch.from_numpy(c[key]).unsqueeze(0) for c in cond]
            tmp_value = torch.cat(tmp_value, dim=0).pin_memory().to(opts.device)
            yield_cond.update({key:tmp_value})
        yield yield_cond
#----------------------------------------------------------------------------
...
def compute_feature_stats_for_generator(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64, batch_gen=None, **stats_kwargs):
    ...
    # cond_iter = iterate_random_cond(opts=opts, batch_size=batch_gen)
    # Initialize.
