...
def gen_interp_video(G, mp4: str, seeds, shuffle_seed=None, w_frames=60*4, kind='cubic', grid_dims=(1,1), num_keyframes=None, wraps=2, psi=1, truncation_cutoff=14, cfg='FFHQ', image_mode='image_depth', gen_shapes=False, device=torch.device('cuda'), **video_kwargs):
    grid_w = grid_dims[0]
    grid_h = grid_dims[1]
    ...
    # zs2 = torch.from_numpy(np.stack([np.random.RandomState(2306).randn(G.z_dim) for seed in all_seeds])).to(device)
    ...
    # ws2 = G.mapping(z=zs2, c=c, truncation_psi=psi, truncation_cutoff=truncation_cutoff)
    # ws = torch.cat([ws[:, :6], ws2[:, 6:]], dim=1)
...
@click.option('--w-frames', type=int, help='Number of frames to interpolate between latents', default=600)
