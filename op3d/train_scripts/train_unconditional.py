...
@click.option('--cbase',        help='Capacity multiplier', metavar='INT',                      type=click.IntRange(min=1), default=24576, show_default=True)
@click.option('--cmax',         help='Max. feature maps', metavar='INT',                        type=click.IntRange(min=1), default=384, show_default=True)
...
@click.option('--plane_number',    help='Number of Planes along each axis', metavar='INT', type=click.IntRange(min=4), default=8, required=True, show_default=True)
@click.option('--fourier_L',    help='Dim of Fourier Encoding of Positions of Planes', metavar='INT', type=click.IntRange(min=0), default=10, required=True, show_default=True)
def main(**kwargs):
    """Train a GAN using the techniques described in the paper
    "Alias-Free Generative Adversarial Networks".
    ...
    # TODO: Here
    # c.G_kwargs.mapping_kwargs.lr_multiplier = 0.005
    ...
    c.G_kwargs.per_dir_plane_number = opts.plane_number 
    c.G_kwargs.fourier_L = opts.fourier_L
    ...
    if opts.resume is not None:
        ...
        c.ada_kimg = 200 # Make ADA react faster at the beginning.
