...
class Dataset(torch.utils.data.Dataset):
    ...
    def get_raw_idx_term(self, raw_idx):
        ...
        if raw_idx == 118464:
            raw_idx += 1
            pose_fname = self._image_fnames[raw_idx].replace(self._image_fnames[raw_idx].split('.')[-1], 'pickle')
        with open(f'./processed_pose_cond/{pose_fname}', 'rb') as f:
            cond = pickle.load(f)
        f.close()
        ...
        return image.copy(), label.copy(), cond
