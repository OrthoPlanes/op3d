...
from training.networks_stylegan2_origin import DiscriminatorBlock, MappingNetwork, DiscriminatorEpilogue
...
@persistence.persistent_class
class DualDiscriminatorWithWarping(torch.nn.Module):
    def __init__(self,
        c_dim,                          # Conditioning label (C) dimensionality.
        img_resolution,                 # Input resolution.
        img_channels,                   # Number of input color channels.
    ...
    def forward(self, img, c, real_conditions, update_emas=False, **block_kwargs):
        image_raw = filtered_resizing(img['image_raw'], size=img['image'].shape[-1], f=self.resample_filter)
        img = torch.cat([img['image'], image_raw], 1)
        ...
        if self.c_dim > 0:
            with torch.no_grad():
                # full_pose = real_conditions['full_pose'][:, 1:, :3, :3].to(c.device)  # [B, 23, 3, 3]
                # shapes = real_conditions['body_shape'].to(c.device)  # [B, 10]
                # pose_c = torch.zeros([shapes.shape[0], 23, 3], device=c.device)
                ...
                full_pose = real_conditions['full_pose'].to(c.device)[:, 1:, :3, :3] # .reshape(-1, 23, 3, 3)  # [B, 23, 3, 3]
                shapes = real_conditions['body_shape'].to(c.device).reshape(-1, 10)  # [B, 10]
                pose_c = torch.zeros([shapes.shape[0], 23, 3], device=c.device)
                pose_c[..., 0] = torch.atan2(full_pose[:, :, 2, 1], full_pose[:, :, 2, 2])
                pose_c[..., 1] = torch.atan2(-full_pose[:, :, 2, 0],
                                             torch.sqrt(full_pose[:, :, 2, 1] ** 2 + full_pose[:, :, 2, 2] ** 2))
            ...
            c = torch.cat([c, shapes, pose_c.reshape(-1, 69)], dim=-1)
            if self.disc_c_noise > 0: c += torch.randn_like(c) * c.std(0) * self.disc_c_noise
            cmap = self.mapping(None, c)
        x = self.b4(x, img, cmap)
        return x
