...
from training.volumetric_rendering.renderer import TriImportanceRenderer
...
class TriMultiPlaneGenerator(torch.nn.Module):
    def __init__(self,
        ...
        self.backbone = StyleGAN2Backbone(z_dim, c_dim, w_dim, img_resolution=256, img_channels=32 * 3, fourier_L=fourier_L,
        ...
        self.n_planes_per_axis = per_dir_plane_number
    ...
    def synthesis(self, ws, c, neural_rendering_resolution=None, update_emas=False, cache_backbone=False,
                  use_cached_backbone=False, feature=True, **synthesis_kwargs):
        ...
        # clone ws copies for parallel generation of orthoplanes
        ws_cloned = ws.clone()
        ...
        return {'image': sr_image, 'image_raw': rgb_image, 'image_depth': depth_image, "ff": f}

    def sample(self, coordinates, directions, ws, c, truncation_psi=1, truncation_cutoff=None, update_emas=False,
               **synthesis_kwargs):
        ...
        coord_embedding = torch.cat([ws_cloned, embedding], dim=-1)
        planes = self.backbone.synthesis(ws, coord_embedding, update_emas=update_emas, **synthesis_kwargs)
        #  planes = planes.view(ws.shape[0], -1, 3, planes.shape[-3], planes.shape[-2], planes.shape[-1])
        planes = planes.view(ws.shape[0], self.n_planes_per_axis, planes.shape[-3], planes.shape[-2], planes.shape[-1])
    ...
    def sample_mixed(self, coordinates, directions, ws, truncation_psi=1, truncation_cutoff=None, update_emas=False,
        ...
        useful_coordinate_point = torch.linspace(-1, 1, self.n_planes_per_axis).to(ws.device). \
            reshape(1, self.n_planes_per_axis, 1). \
            expand(ws.shape[0], self.n_planes_per_axis, 1)
        # useful_coordinate_point = useful_coordinate_point.reshape(-1, 1)
        useful_coordinate_point = useful_coordinate_point.reshape(ws.shape[0] * self.n_planes_per_axis)
        # coordinate based feature
        embedding = torch.zeros([useful_coordinate_point.shape[0], self.L * 2 + 1], device=useful_coordinate_point.device)
        embedding[..., 0] = useful_coordinate_point
