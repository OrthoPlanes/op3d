...
def setup_snapshot_image_grid_with_warping(training_set, random_seed=0):
    ...
    gh = 6 # np.clip(4320 // training_set.image_shape[1], 4, 32)
    ...
    for key in conditions[0].keys():
        ...
        for cond in conditions:
            batch_cond.append(torch.from_numpy(cond[key]).unsqueeze(0))
...
def training_loop_with_warping(
    ...
    np.random.seed(random_seed * num_gpus + rank + 1025)
    torch.manual_seed(random_seed * num_gpus + rank)
    torch.backends.cudnn.benchmark = cudnn_benchmark    # Improves training speed.
    torch.backends.cuda.matmul.allow_tf32 = False       # Improves numerical accuracy.
    torch.backends.cudnn.allow_tf32 = False             # Improves numerical accuracy.
    ...
    if rank == 0:
        print('Exporting sample images...')
        grid_size, images, labels, conditions = setup_snapshot_image_grid_with_warping(training_set=training_set, random_seed=random_seed * num_gpus + rank + 1025)
        save_image_grid(images, os.path.join(run_dir, 'reals.png'), drange=[0,255], grid_size=grid_size)
        grid_z = torch.randn([labels.shape[0], G.z_dim], device=device).split(batch_gpu)
        grid_c = torch.from_numpy(labels).to(device).split(batch_gpu)
    ...
    while True:
        ...
        for phase, phase_gen_z, phase_gen_c in zip(phases, all_gen_z, all_gen_c):
            ...
            phase.module.requires_grad_(True)
            for real_img, real_c, gen_z, gen_c in zip(phase_real_img, phase_real_c, phase_gen_z, phase_gen_c):
                loss.accumulate_gradients(phase=phase.name, real_img=real_img, real_c=real_c, real_condition=phase_real_condition, gen_z=gen_z, gen_c=gen_c, gain=phase.interval, cur_nimg=cur_nimg)
            phase.module.requires_grad_(False)
            torch.nn.utils.clip_grad_norm_(phase.module.parameters(), 5.)
            # Update weights.
            with torch.autograd.profiler.record_function(phase.name + '_opt'):
                params = [param for param in phase.module.parameters() if param.numel() > 0 and param.grad is not None]
                if len(params) > 0:
