...
'''
#---------------------------------------------------------------------------
class StyleGAN2LossWithWarping(Loss):
    def __init__(self, device, G, D, augment_pipe=None, r1_gamma=10, style_mixing_prob=0, pl_weight=0, pl_batch_shrink=2, pl_decay=0.01, pl_no_weight_grad=False, blur_init_sigma=0, blur_fade_kimg=0, r1_gamma_init=0, r1_gamma_fade_kimg=0, neural_rendering_resolution_initial=64, neural_rendering_resolution_final=None, neural_rendering_resolution_fade_kimg=0, gpc_reg_fade_kimg=1000, gpc_reg_prob=None, dual_discrimination=False, filter_mode='antialiased'):
        super().__init__()
        self.device             = device
        self.G                  = G
    ...
    def run_G(self, z, c, real_condition, swapping_prob, neural_rendering_resolution, update_emas=False):
        if swapping_prob is not None:
            c_swapped = torch.roll(c.clone(), 1, 0)
            c_gen_conditioning = torch.where(torch.rand((c.shape[0], 1), device=c.device) < swapping_prob, c_swapped, c)
        else:
        ...
        if self.style_mixing_prob > 0:
            with torch.autograd.profiler.record_function('style_mixing'):
                ...
                cutoff = torch.where(torch.rand([], device=ws.device) < self.style_mixing_prob, cutoff, torch.full_like(cutoff, ws.shape[1]))
                ws[:, cutoff:] = self.G.mapping(torch.randn_like(z), c, update_emas=False)[:, cutoff:]
        gen_output = self.G.synthesis(ws, c, real_condition, neural_rendering_resolution=neural_rendering_resolution, update_emas=update_emas)
        return gen_output, ws

    def run_D(self, img, c, real_condition, blur_sigma=0, blur_sigma_raw=0, update_emas=False):
        blur_size = np.floor(blur_sigma * 3)
        if blur_size > 0:
            with torch.autograd.profiler.record_function('blur'):
                f = torch.arange(-blur_size, blur_size + 1, device=img['image'].device).div(blur_sigma).square().neg().exp2()
        ...
        logits = self.D(img, c, real_condition, update_emas=update_emas)
        return logits

    def accumulate_gradients(self, phase, real_img, real_c, real_condition, gen_z, gen_c, gain, cur_nimg):
        assert phase in ['Gmain', 'Greg', 'Gboth', 'Dmain', 'Dreg', 'Dboth']
        if self.G.rendering_kwargs.get('density_reg', 0) == 0:
            phase = {'Greg': 'none', 'Gboth': 'Gmain'}.get(phase, phase)
        if self.r1_gamma == 0:
        ...
        # if cur_nimg == 2400:
        #     self.r1_gamma = self.r1_gamma // 2
        if cur_nimg <= 1600 * 1e3:
            cond = copy.deepcopy(real_condition)
            for key, value in cond.items():
                cond[key] = torch.zeros_like(value, device=value.device)
        else:
            cond = real_condition
        
        if cur_nimg <= 1600 * 1e3:
            c = gen_c
        elif cur_nimg <= 2400 * 1e3:
            c = real_c
        else:
            c = gen_c
        ...
        if phase in ['Gmain', 'Gboth']:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                gen_img, _gen_ws = self.run_G(gen_z, c, real_condition, swapping_prob=swapping_prob, neural_rendering_resolution=neural_rendering_resolution)
                gen_logits = self.run_D(gen_img, c, cond, blur_sigma=blur_sigma)
                training_stats.report('Loss/scores/fake', gen_logits)
                training_stats.report('Loss/signs/fake', gen_logits.sign())
                loss_Gmain = torch.nn.functional.softplus(-gen_logits)
                training_stats.report('Loss/G/loss', loss_Gmain)
        ...
        if phase in ['Greg', 'Gboth'] and self.G.rendering_kwargs.get('density_reg', 0) > 0 and self.G.rendering_kwargs['reg_type'] == 'l1':
            ...
            perturbed_coordinates = initial_coordinates + torch.randn_like(initial_coordinates) * self.G.rendering_kwargs['density_reg_p_dist']
            all_coordinates = torch.cat([initial_coordinates, perturbed_coordinates], dim=1)
            sigma = self.G.sample_mixed(all_coordinates, torch.randn_like(all_coordinates), ws, real_condition, update_emas=False)['sigma']
            sigma_initial = sigma[:, :sigma.shape[1]//2]
            sigma_perturbed = sigma[:, sigma.shape[1]//2:]
        ...
        if phase in ['Dmain', 'Dboth']:
            with torch.autograd.profiler.record_function('Dgen_forward'):
                gen_img, _gen_ws = self.run_G(gen_z, c, real_condition, swapping_prob=swapping_prob, neural_rendering_resolution=neural_rendering_resolution, update_emas=True)
                gen_logits = self.run_D(gen_img, c, cond, blur_sigma=blur_sigma, update_emas=True)
                training_stats.report('Loss/scores/fake', gen_logits)
                training_stats.report('Loss/signs/fake', gen_logits.sign())
                loss_Dgen = torch.nn.functional.softplus(gen_logits)
            with torch.autograd.profiler.record_function('Dgen_backward'):
        ...
        if phase in ['Dmain', 'Dreg', 'Dboth']:
            ...
            with torch.autograd.profiler.record_function(name + '_forward'):
                ...
                real_logits = self.run_D(real_img_tmp, real_c, cond, blur_sigma=blur_sigma)
                training_stats.report('Loss/scores/real', real_logits)
                training_stats.report('Loss/signs/real', real_logits.sign())
...
class StyleGAN2LossWithWarpingSDF(Loss):
    def __init__(self, device, G, D, augment_pipe=None, r1_gamma=10, style_mixing_prob=0, pl_weight=0, pl_batch_shrink=2, pl_decay=0.01, pl_no_weight_grad=False, blur_init_sigma=0, blur_fade_kimg=0, r1_gamma_init=0, r1_gamma_fade_kimg=0, neural_rendering_resolution_initial=64, neural_rendering_resolution_final=None, neural_rendering_resolution_fade_kimg=0, gpc_reg_fade_kimg=1000, gpc_reg_prob=None, dual_discrimination=False, filter_mode='antialiased'):
        super().__init__()
        self.device             = device
        self.G                  = G
    ...
    def accumulate_gradients(self, phase, real_img, real_c, real_condition, gen_z, gen_c, gain, cur_nimg):
        ...
        if cur_nimg <= 200 * 1e3:
            ...
            for key, value in cond.items():
                cond[key] = torch.zeros_like(value, device=value.device)
            c = gen_c
        elif cur_nimg <= 400 * 1e3:
            cond = real_condition
            c = real_c
        elif cur_nimg <= 800 * 1e3:
            cond = copy.deepcopy(real_condition)
            for key, value in cond.items():
                cond[key] = torch.zeros_like(value, device=value.device)
            c = gen_c
        elif cur_nimg <= 1200 * 1e3:
            cond = real_condition
            c = real_c
        else:
            cond = real_condition
        ...
        if phase in ['Gmain', 'Gboth']:
            ...
            # sdf-related loss
            all_sdf_information = gen_img['sdf']

            # prior loss
            sdf = all_sdf_information['sdf'].reshape(gen_img['image'].shape[0], -1, 1)
            sdf_zero = all_sdf_information['sdf_zero'].reshape(gen_img['image'].shape[0], -1, 1)
            delta_sdf = sdf - sdf_zero

            # omega = torch.exp(-sdf_zero ** 2 / 10)
            prior_loss = (delta_sdf) ** 2
            prior_loss = prior_loss.mean() * 1.
            training_stats.report('Loss/G/prior_loss', prior_loss)

            prior_loss.mul(gain).backward(retain_graph=True)

            # eik loss
            eik_loss = all_sdf_information['eik_grad']
            eik_loss = ((eik_loss) ** 2 - 1) ** 2
            # eik_loss = eik_loss.mean() * 1e-3
            eik_loss = eik_loss.mean()
            training_stats.report('Loss/G/eik_loss', eik_loss)
            eik_loss = eik_loss.mean() * 1e-3

            eik_loss.mul(gain).backward()

            # min surf
            # min_surf_loss = torch.exp(-100 * sdf.abs())
            # min_surf_loss = min_surf_loss.mean()
            # training_stats.report('Loss/G/min_surf_loss', min_surf_loss)
            # min_surf_loss = min_surf_loss * 0.05
            # min_surf_loss.mul(gain).backward()
...
from training.normal_utils import depth_to_normal
class StyleGAN2LossWithWarpingNormal(Loss):
    def __init__(self, device, G, D, Normal_Branch,augment_pipe=None, r1_gamma=10, style_mixing_prob=0, pl_weight=0,
    ...
    def run_G(self, z, c, real_condition, swapping_prob, neural_rendering_resolution, update_emas=False):
        ...
        if self.style_mixing_prob > 0:
            with torch.autograd.profiler.record_function('style_mixing'):
                ...
                ws[:, cutoff:] = self.G.mapping(torch.randn_like(z), c, update_emas=False)[:, cutoff:]
        gen_output = self.G.synthesis(ws, c, real_condition, neural_rendering_resolution=neural_rendering_resolution,
    ...
    def accumulate_gradients(self, phase, real_img, real_c, real_condition, gen_z, gen_c, gain, cur_nimg):
        ...
        if phase in ['Gmain', 'Gboth']:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                gen_img, _gen_ws = self.run_G(gen_z, c, real_condition, swapping_prob=swapping_prob,
            ...
            if cur_nimg > 400 * 1e3:
                with torch.autograd.profiler.record_function('G_Geo_Branch_forward'):
                    gen_img, _ = self.run_G(gen_z, real_c, real_condition, swapping_prob=swapping_prob,
                                                  neural_rendering_resolution=neural_rendering_resolution)
                    if neural_rendering_resolution == 64 or neural_rendering_resolution == 128:
                        img_input_normal_branch = gen_img['image_raw']
                        depth_raw = gen_img['image_depth']
                    else:
                        img_input_normal_branch = torch.nn.functional.interpolate(
                            gen_img['image_raw'],
                            (64, 64)
                        )
                    ...
                    depth_raw = depth_raw.to(torch.uint8)[:, 0]  # B, C, H, W -> B, H, W
                    normal_raw = depth_to_normal(depth_raw)  # normal computed

                    gt_smpl_normal = torch.nn.functional.interpolate(
                        real_condition['smpl_normal'],
                        (64, 64)
                    ).to(torch.float) if neural_rendering_resolution < 128 else torch.nn.functional.interpolate(
                        real_condition['smpl_normal'],
                        (128, 128)
                    ).to(torch.float)
                    gt_smpl_normal = gt_smpl_normal / (torch.norm(gt_smpl_normal, dim=1, keepdim=True) + 1e-12)
                    gt_smpl_normal = gt_smpl_normal.to(img_input_normal_branch.device)
                    # human mask
                    human_mask = torch.where(((gt_smpl_normal ** 2).sum(dim=1, keepdim=True) - 1).abs() < 1e-12, 1, 0)
                    detected_normal = self.Normal_Branch(torch.cat([
                        img_input_normal_branch,
                        gt_smpl_normal
                    ], dim=1))
                    detected_normal = detected_normal / (torch.norm(detected_normal, dim=1, keepdim=True) + 1e-12)

                    normal_loss = human_mask * ( (detected_normal - normal_raw) ** 2).sum(dim=1, keepdim=True)
                    normal_loss = normal_loss.mean()
                    training_stats.report('Loss/G/Normal_Loss', normal_loss)

                    normal_loss = 0.1 * normal_loss

                with torch.autograd.profiler.record_function('G_Geo_Branch_backward'):
                    normal_loss.mul(gain).backward()
        ...
        if phase in ['Greg', 'Gboth'] and self.G.rendering_kwargs.get('density_reg', 0) > 0 and self.G.rendering_kwargs[
            ...
            all_coordinates = torch.cat([initial_coordinates, perturbed_coordinates], dim=1)
            sigma = self.G.sample_mixed(all_coordinates, torch.randn_like(all_coordinates), ws, real_condition,
        ...
        if phase in ['Greg', 'Gboth'] and self.G.rendering_kwargs.get('density_reg', 0) > 0 and self.G.rendering_kwargs[
            ...
            perturbed_coordinates = initial_coordinates + torch.tensor([0, 0, -1], device=ws.device) * (1 / 256) * \
            ...
            if self.style_mixing_prob > 0:
                with torch.autograd.profiler.record_function('style_mixing'):
                    ...
                    ws[:, cutoff:] = self.G.mapping(torch.randn_like(z), c, update_emas=False)[:, cutoff:]
            initial_coordinates = torch.rand((ws.shape[0], 1000, 3), device=ws.device) * 2 - 1
            perturbed_coordinates = initial_coordinates + torch.randn_like(initial_coordinates) * (1 / 256) * \
        ...
        if phase in ['Dmain', 'Dreg', 'Dboth']:
            ...
            # normal branch
            with torch.autograd.profiler.record_function('D_Geo_Branch_forward'):
                if neural_rendering_resolution == 64 or neural_rendering_resolution == 128:
                    img_input_normal_branch = real_img['image_raw'].detach().requires_grad_(False)
                else:
                    img_input_normal_branch = torch.nn.functional.interpolate(
                        real_img['image_raw'].detach().requires_grad_(False), (64, 64))
                ...
                gt_smpl_normal = gt_smpl_normal / (torch.norm(gt_smpl_normal, dim=1, keepdim=True) + 1e-12)
                gt_smpl_normal = gt_smpl_normal.to(img_input_normal_branch.device)
                normal_raw = torch.nn.functional.interpolate(
                    real_condition['normal'],
                    (64, 64)
                ).to(torch.float) if neural_rendering_resolution < 128 else torch.nn.functional.interpolate(
                    real_condition['normal'],
                    (128, 128)
                ).to(torch.float)
                normal_raw = normal_raw / (torch.norm(normal_raw, dim=1, keepdim=True) + 1e-12)
                normal_raw = normal_raw.to(img_input_normal_branch.device)
                # human mask
                # print(img_input_normal_branch.device, gt_smpl_normal.device)
                human_mask = torch.where(((gt_smpl_normal ** 2).sum(dim=1, keepdim=True) - 1).abs() < 1e-12, 1, 0)
                detected_normal = self.Normal_Branch(torch.cat([
                    img_input_normal_branch,
                    gt_smpl_normal
                ...
                normal_loss = human_mask * ( (detected_normal - normal_raw) ** 2).sum(dim=1, keepdim=True)
                normal_loss = normal_loss.mean()
                training_stats.report('Loss/D/Normal_Loss', normal_loss)

                normal_loss = normal_loss

            with torch.autograd.profiler.record_function('D_Geo_Branch_backward'):
                normal_loss.mul(gain).backward()
'''
